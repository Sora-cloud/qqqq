...
class PersonalRoles(commands.Cog, View):
    ...
    async def create(
        ...
        if self.db.get_balance(ctx.author.id) >= self.cost_role_create:
            ...
            embed = discord.Embed(title=f"Создание роли", description=f"**Вы** уверены что хотите создать роль **{name}** за **{self.cost_role_create}** :candy:?", color=0x2f3136)
                            ...
                            class ButtonsEditView(discord.ui.View):
                                ...
                                async def button_callback_change_name(self, button, interaction: discord.Interaction):
                                    ...
                                    if ctx.author.id == interaction.user.id:
                                        ...
                                        embed = discord.Embed(title=f'Изменение названия роли — {ctx.author.name}#{ctx.author.discriminator}', description=f"{ctx.author.mention}, **Вы уверены** что хотите изменить **название** роли на **{message.content}** за **{globalself.cost_role_change_name}** :candy:?", color=0x2f3136)
                                        embed.set_thumbnail(url=ctx.author.display_avatar.url)
                                ...
                                async def button_callback_change_color(self, button, interaction: discord.Interaction):
                                    ...
                                    if ctx.author.id == interaction.user.id:
                                        ...
                                        embed = discord.Embed(title=f'Изменение цвета роли', description=f"**Вы уверены** что хотите изменить **цвет** роли на **{message.content}** за **{globalself.cost_role_change_color}** :candy:?", color=0x2f3136)
                                        embed.set_thumbnail(url=ctx.author.display_avatar.url)
