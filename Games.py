        ...
        if globalself.utils.is_active_game(ctx.author.id):
            ...
            if globalself.db.get_balance(ctx.author.id) >= amount:
                ...
                if game == "Орёл или Решка":
                    ...
                    embed = discord.Embed(title=f'{game} — {ctx.author.name}#{ctx.author.discriminator}', description=f"{ctx.author.mention}, выберите сторону на которую **хотите** поставить ваши **{amount}** :candy:", color=0x2f3136)
                    embed.set_thumbnail(url=ctx.author.display_avatar.url)
                    ...
                    class ButtonsView(discord.ui.View):
                        ...
                        async def button_callback_orel(self, button, interaction: discord.Interaction):
                            ...
                            if ctx.author.id == interaction.user.id:
                                ...
                                embed = discord.Embed(title=f'{game} — {ctx.author.name}#{ctx.author.discriminator}', description=f"**Ставка:** {amount} :candy:\n**Выбранная сторона**: Орёл", color=0x2f3136)
                                ...
                                if random_win == 'Орёл':
                                    embed = discord.Embed(title=f'{game} — {ctx.author.name}#{ctx.author.discriminator}', description=f"{interaction.user.mention}, выпал **Орёл**, **Вы** выиграли **{amount}** :candy:", color=0x2f3136)
                                    embed.set_thumbnail(url=ctx.author.display_avatar.url)
                                    ...
                                    embed = discord.Embed(title=f'{game} — {ctx.author.name}#{ctx.author.discriminator}', description=f"{interaction.user.mention}, выпала **Решка**, **Вы** проиграли **{amount}** :candy:", color=0x2f3136)
                                    embed.set_thumbnail(url=ctx.author.display_avatar.url)
                                    embed.set_footer(text=f"Ваш баланс — {globalself.db.get_balance(ctx.author.id)}")
                        ...
                        async def button_callback_reshka(self, button, interaction: discord.Interaction):
                            ...
                            if ctx.author.id == interaction.user.id:
                                ...
                                if random_win == 'Решка':
                                    embed = discord.Embed(title=f'{game} — {ctx.author.name}#{ctx.author.discriminator}', description=f"{interaction.user.mention}, выпала **Решка**, **Вы** выиграли **{amount}** :candy:", color=0x2f3136)
                                    embed.set_thumbnail(url=ctx.author.display_avatar.url)
                                    ...
                                    embed = discord.Embed(title=f'{game} — {ctx.author.name}#{ctx.author.discriminator}', description=f"{interaction.user.mention}, выпал **Орёл**, **Вы** проиграли **{amount}** :candy:", color=0x2f3136)
                                    embed.set_thumbnail(url=ctx.author.display_avatar.url)
                                    embed.set_footer(text=f"Ваш баланс — {globalself.db.get_balance(ctx.author.id)}")
