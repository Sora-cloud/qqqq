...
class Profile(commands.Cog, View):
    ...
    async def generate_profile(self, member):
        ...
        # Ник
        editor.text((1461, 390), f"{member_name}", color="white", font=font_60, align="center")
        ...
        if (member):
                            ...
                            if ctx.author.id == interaction.user.id:
                                ...
                                if f"theme_{id_theme}" in self.user_themes:
                                    ...
                                    if globalself.db.get_balance(ctx.author.id) >= cost:
                                            ...
                                            async def button_callback_yes(self, button, interaction: discord.Interaction):
                                                ...
                                                if ctx.author.id == interaction.user.id:
                                                    if f"theme_{id_theme}" in self.user_themes:
                                                        ...
                                                        embed = discord.Embed(title=f'Магазин профилей', description=f"**Вы** успешно **купили** тему профиля **№{id_theme}** за **{globalself.cost_theme_default_profile}** :candy:\n\nНовая тема **установлена**", color=discord.Colour.green())
                                                        embed.set_image(url="https://cdn.discordapp.com/attachments/992883178362642453/1029462389130792970/-1.png")
                                                        embed.set_thumbnail(url=ctx.author.display_avatar.url)
                                        ...
                                        embed = discord.Embed(title=f'Магазин профилей', description=f"**Вы** уверены что хотите **купить** тему профиля **№{id_theme}** за **{globalself.cost_theme_default_profile}** :candy:", color=0x2f3136)
                                        embed.set_image(url="https://cdn.discordapp.com/attachments/992883178362642453/1029462389130792970/-1.png")
                                        embed.set_thumbnail(url=ctx.author.display_avatar.url)
            ...
            class ProfileMenu(discord.ui.View):
                def __init__(self, *, timeout=120):
                    ...
                    if not member:
                        button_change_profile = discord.ui.Button(label="Изменить профиль", style=discord.ButtonStyle.gray, custom_id="button_change_profile")
